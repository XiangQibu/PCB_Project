...
def box_iou(a, b):
    return box_intersection(a, b) / (box_union(a, b) + 1e-10)
...
def do_kmeans(n_anchors, boxes, centroids):
    ...
    for i in range(n_anchors):
        groups.append([])
        new_centroids.append(Box(float(0), float(0), float(0), float(0)))
    ...
    for i in range(n_anchors):
        new_centroids[i].w /= (len(groups[i]) + 1e-10)
        new_centroids[i].h /= (len(groups[i]) + 1e-10)
...
def compute_centroids(label_path,n_anchors,loss_convergence,grid_size,iterations_num,plus):
    ...
    for label_file in file_path_list:
        f = open(label_file)
        for line in f:
            temp = line.strip().split(" ")
            if len(temp) > 1:
                boxes.append(Box(float(0), float(0), float(temp[3]), float(temp[4])))
    ...
    for centroid in centroids:
        ...
        # print(centroid.w * grid_size * 32, centroid.h * grid_size * 32)
        centroid_list.append([centroid.w * grid_size * 32, centroid.h * grid_size * 32])
...
if __name__ == '__main__':
    ...
    for i in range(5):
        print("epoch: ",i)
        print("k-means result:\n")
        centroid_list = compute_centroids(label_path,n_anchors,loss_convergence,grid_size,iterations_num,plus)
        centroid_list.sort(key= lambda x:x[0])
        anchor_list = [[round(num) for num in sublst] for sublst in centroid_list]
        print(anchor_list)
        centroid_list_list.append(anchor_list)